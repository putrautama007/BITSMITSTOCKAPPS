...
class BarangAdapter(val lisBarang: ArrayList<Barang>) : RecyclerView.Adapter<BarangAdapter.MyViewHolder>(){
    ...
    override fun getItemCount(): Int {
        return lisBarang.size
    }

    override fun onBindViewHolder(holder: MyViewHolder?, position: Int) {
        holder?.namaBarangTv?.text = lisBarang.get(position).nama_barang

        val harga = "Rp "+lisBarang.get(position).harga
        holder?.hargaBarangTv?.text = lisBarang.get(position).harga

        holder?.stockBarangTv?.text = lisBarang.get(position).jumlah
    }
